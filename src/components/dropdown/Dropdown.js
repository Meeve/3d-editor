...
export default class Dropdown extends React.Component {
    ...
    calculateStyleForRightDirection = (dropdownSizes, opennerSizes) => {
        return {
            top: opennerSizes.top + dropdownSizes.height > window.innerHeight ? -dropdownSizes.height + opennerSizes.height + "px" : 0,
    ...
    styleChild = (child) => {
        return <div className="dropdownMenuAction">{child}</div>;
    }
    ...
    render() {
        return (<div className="dropdown" ref={this.state.opennerHolderRef} onMouseLeave={() => this.setState({isOpened: false})}>
            <div className="dropdownContent" {...this.state.openActions}>
                {this.props.content}
            </div>
            {this.getExpandedDropdown()}

            <div className="dropdownEventCatcher" style={this.getStylesForEventCatcher()}></div>
        </div>);
    }
}