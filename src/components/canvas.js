...
class Canvas extends Component {
  ...
  componentDidMount() {
    ...
    mat4.perspective(this.state.projectionMatrix, 45, 600 / 400, 0.1, 100.0);
  ...
  drawScene() {
    this.state.gl.viewport(0, 0, 600, 400);
  ...
  render() {
    ...
    return (
      <div ref={(ref) => this.canvas = ref} style={ {overflow: "hidden" } } >
        <canvas 
            onMouseDown={ e => {this.setState({ mouseDown: true, clickedX: e.pageX, prevXValue: this.state.xOffset });} } 
            onMouseMove={ this.mouseMove.bind(this) }
            onMouseLeave={ e => this.setState({ isMouseIn: false }) }
            onMouseEnter={ e => this.setState({ isMouseIn: true }) }
            id={this.state.holder} 
            width={ this.canvas ? this.canvas.offsetWidth + "px" : 0 } 
            height={ this.canvas ? this.canvas.offsetHeight + "px" : 0}></canvas>
      </div>
    );
  }
}
