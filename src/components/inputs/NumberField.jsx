...
export const NumberField = ({value, label, slideSpeed, onChange, size = 'medium'}) => {
   ...
   return (
      <React.Fragment>
         {!state.id && <div className="numberFieldHolder" 
            style={{width: sizeMap[size]}}
            >
            <div className={"numberField " + state.classes}>
               <div className="arrow leftArrow" onClick={e => { e.stopPropagation(); onChange(parseFloat(value) - 1)}}></div>
               ...
               <div className="arrow rightArrow" onClick={e => { e.stopPropagation(); onChange(parseFloat(value) + 1)}}></div>
            </div>
         </div>}
