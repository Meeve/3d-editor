  ...
  render() {
    const grid = _.map(this.state.components, (el, key) => {
      ...
      const diagonalResizer = colCounter != this.state.columns.length - 1 && rowCounter != this.state.rows.length - 1 ?
        <div className="diagonalResizer" onMouseDown={this.diagonallyResize.bind(this, rowCounter, colCounter)}></div> : "";
      ...
      return <div className="resizerBox" style={ resizerStyle }>
        ...
        <div className="multiplayer" onMouseDown={this.multiplayerMouseDown.bind(this)} onMouseLeave={this.multiplayerMouseLeave.bind(this, el)}></div>
      </div>
    });

    return (
      <div className="app" onMouseMove={this.appResize.bind(this)} onMouseUp={this.stopResizing.bind(this)} style={ this.state.appStyles }>
          {grid}
      </div>
    );
  }
